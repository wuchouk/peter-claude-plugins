...
`first_marked_at`／`first_marked_head` 兩個欄位**繼續寫入**，gate 不再讀。留著的理由是升級過渡：
...
屆時 `first_marked_at` 可以清掉；`first_marked_head` 不行，它是界定「這一輪」的唯一依據。
...
`first_marked_at` = **這一輪**最早打勾的時間（重打時沿用），`first_marked_head` = 寫下它時的 `HEAD`，「這一輪」就是靠它界定的——commit 會推進 HEAD，對不上就是另一輪。2026-09-20 起 **gate 兩個都不讀**，只有 `pipeline-mark-done.sh` 寫入；留著的理由見上方專節末段。
